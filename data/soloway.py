...
class Soloway:
    ...
    def genRules(self):
        ...
        for s in self.suits:
            ...
            for js in jTargets:
                l = 2 if (self.suits.index(s) < self.suits.index(js)) else 1
                l += 1
    ...
    def sjsCriteria(self, s, js):
        ...
        cStrings.append(f"{{'HCP': [13, 16], 'SuitLen': {{'{js}': 6}}, 'Honors': 3, {self.meta}}}")
        cStrings.append(f"{{'HCP': [17, 19], 'SuitLen': {{'{js}': 5}}, 'Shape': 'Balanced', {self.meta}}}")
        cStrings.append(f"{{'HCP': 16, 'SuitLen': {{'{js}': 5, {s}: 4}}, 'Honors': {{'{js}': 2}}, {self.meta}}}")
        for str in cStrings:
            print(f"\t\t{str}", end='')
    ...
    def rebid(self):
        ...
        for s in self.suits:
            ...
            for js in jTargets:
                ...
                b = f"'{l+1}{js}'"
                rebids.append(b)
                print(f"\t{{'Bid': {b}, 'Criteria': [{{'SuitLen': 3, 'Honors': 1, {self.meta}}}]}},")
                b = f"'{l + (0 if self.suits.index(s) < self.suits.index(js) else 1)}{s}'"
                rebids.append(b)
                print(f"\t{{'Bid': {b}, 'Criteria': [{{'SuitLen': 5, 'Honors': 2, {self.meta}}}]}},")
                newSuits = [y for y in jTargets if y != js]
                for ns in newSuits:
                    nsLevel = l + (0 if self.suits.index(ns) < self.suits.index(js) else 1)
                    b = f"'{nsLevel}{ns}'"
                    rebids.append(b)
                    print(f"\t{{'Bid': {b}, 'Criteria': [{{'Honors': 1, {self.meta}}}]}},")
                rebids.append('{l}NT')
                print(f"\t{{'Bid': '{l}NT', 'Criteria': [{{'Stopper': {newSuits}, 'Shape': 'Balanced', {self.meta}}}]}}],")
                print(f"\n\t'Seq': ['1{s}', '-', '{l}{js}', '-']}},")
        return rebids
    ...
    def sjsrebid(self):
        ...
        for s in self.suits:
            ...
            for js in jTargets:
                ...
                rebids.append(f'{l + (0 if self.suits.index(s) < self.suits.index(js) else 1)}{s}')
                newSuits = [y for y in jTargets if y != js]
                for ns in newSuits:
                    nsLevel = l + (0 if self.suits.index(ns) < self.suits.index(js) else 1)
                ...
                for rebid in rebids:
                    ...
                    jsLevel = rLevel + (1 if rSuit != 'NT' and self.suits.index(js) > self.suits.index(rSuit) else 0)
                    ...
                    print(f"\t'Seq': ['1{s}', '-', '{l}{js}', '-', '{rebid}', '-']}},", end='')
